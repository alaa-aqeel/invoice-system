...
class BillDeleteView(DeleteView):# {
    ...
    def dispatch(self, request, *args, **kwargs):# { 
        ...
        current_object.canceled_at = timezone.now()
        current_object.save()
...
class BillProductCreateView(CreateView):# { 
    ...
    def form_valid(self, form):# { 
        ...
        is_not_update = self.update_quantity_product(form)

        if is_not_update != None:
            form.errors.update({
                "quantity": [u"You have %s of the product"%is_not_update]
            })
